...
#define MANUFACTURE 0
  ...
  // Border Waveform Control
  // 0x00 - VBD: GS Transition (Follow LUT, LUT0)
  epd_cmd(0x3C, 0x00);
  // Greyscale
  // https://github.com/ZinggJM/GxEPD2_4G/blob/980148e/src/gdey/GxEPD2_154_GDEY0154D67.cpp
  // XXX: Incorrect
static const uint8_t waveform[] =
{
  0x20, 0x48, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, // VS L0 (red 0, black 0) black
  0x02, 0x48, 0x04, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, // VS L1 (red 1, black 0) dark grey
  0x08, 0x48, 0x10, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, // VS L2 (red 0, black 1) light grey
  0x40, 0x48, 0x80, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, // VS L3 (red 1, black 1) white
  ...
  epd_waitbusy();
  // Write LUT register
  _epd_cmd(0x32, waveform, 153);
  // Write RAM
  for (int r = 80; r < 120; r++)
    for (int c = 11; c < 16; c++)
      pixels[r * 200 / 8 + c] = (c % 2 == 1 ? 0xff : 0x00);
  _epd_cmd(0x24, pixels, sizeof pixels);
  for (int r = 80; r < 120; r++)
    for (int c = 11; c < 16; c++)
      pixels[r * 200 / 8 + c] = (c % 2 == 1 ? 0x00 : 0xff);
  for (int r = 80; r < 120; r++)
    for (int c = 16; c < 21; c++)
